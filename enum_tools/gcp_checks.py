...
def print_bucket_response(reply):
    ...
    if reply.status_code == 404:
        ...
        json_utils.JSON_RESULT["gcp"]["bucket"]["open"].append(reply.url)
        ...
        utils.fmt_output(data)
        json_utils.JSON_RESULT["gcp"]["bucket"]["protected"].append(reply.url)
    else:
...
def print_fbrtdb_response(reply):
    ...
    if reply.status_code == 404:
        ...
        data['access'] = 'public'
        utils.fmt_output(data)
        json_utils.JSON_RESULT["gcp"]["firebase"]["open"].append(reply.url)
    elif reply.status_code == 401:
        data['msg'] = 'Protected Google Firebase RTDB'
        data['target'] = reply.url
        data['access'] = 'protected'
        utils.fmt_output(data)
        json_utils.JSON_RESULT["gcp"]["firebase"]["protected"].append(reply.url)
    elif reply.status_code == 402:
        data['msg'] = 'Payment required on Google Firebase RTDB'
        data['target'] = reply.url
        data['access'] = 'disabled'
        utils.fmt_output(data)
        json_utils.JSON_RESULT["gcp"]["firebase"]["payment"].append(reply.url)
    elif reply.status_code == 423:
        data['msg'] = 'The Firebase database has been deactivated.'
        data['target'] = reply.url
        data['access'] = 'disabled'
        utils.fmt_output(data)
        json_utils.JSON_RESULT["gcp"]["firebase"]["disabled"].append(reply.url)
    else:
        print(f"    Unknown status codes being received from {reply.url}:\n"
              "       {reply.status_code}: {reply.reason}")
...
def print_appspot_response(reply):
    ...
    if reply.status_code == 404:
        ...
        if 'accounts.google.com' in reply.url:
            ...
            data['access'] = 'protected'
            utils.fmt_output(data)
            json_utils.JSON_RESULT["gcp"]["appspot"]["protected"].append(reply.url)
        else:
            data['msg'] = 'Open Google App Engine app'
            data['target'] = reply.url
            data['access'] = 'public'
            utils.fmt_output(data)
            json_utils.JSON_RESULT["gcp"]["appspot"]["open"].append(reply.url)
    else:
        print(f"    Unknown status codes being received from {reply.url}:\n"
              "       {reply.status_code}: {reply.reason}")
...
def print_functions_response1(reply):
    ...
    if reply.status_code == 404:
        ...
        data['access'] = 'public'
        utils.fmt_output(data)
        json_utils.JSON_RESULT["gcp"]["function"]["viewed"].append(reply.url)
...
def print_functions_response2(reply):
    ...
    if 'accounts.google.com/ServiceLogin' in reply.url:
        ...
        data['access'] = 'protected'
        utils.fmt_output(data)
        json_utils.JSON_RESULT["gcp"]["function"]["authRequired"].append(reply.url)
    elif reply.status_code == 405:
        data['msg'] = 'UNAUTHENTICATED Cloud Function (POST-Only)'
        data['target'] = reply.url
        data['access'] = 'public'
        json_utils.JSON_RESULT["gcp"]["function"]["post"].append(reply.url)
        utils.fmt_output(data)
    elif reply.status_code in (200, 404):
        data['msg'] = 'UNAUTHENTICATED Cloud Function (GET-OK)'
        data['target'] = reply.url
        data['access'] = 'public'
        json_utils.JSON_RESULT["gcp"]["function"]["get"].append(reply.url)
        utils.fmt_output(data)
    else:
        print(f"    Unknown status codes being received from {reply.url}:\n"
              "       {reply.status_code}: {reply.reason}")
